...
@bot.command()
async def help_me():
    await bot.say("Here's what I can do for you")
    await bot.say("Commands:")  
    await bot.say("?add_me - adds you to the bot's list of dads where it can track things like rs availability and other features coming soon")
    await bot.say("?set_rs_level x - sets your maximum rs level to x")
    await bot.say("?free_for x y - lets the bot know that you are up free for red star searches for x hours and y minutes")
    await bot.say("?whos_free - lists the dads who are currently free for red star searches")
    await bot.say("?ping_free - pings all of the dads who are free for red star searches")
#    await bot.say("?who_am_i - idk man, who are you tho")
#    await bot.say("?next_salvo - check when the next salvo of rockets is to be sent off")
